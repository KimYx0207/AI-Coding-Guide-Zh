...
> **v2.1.136→v2.1.150 企业治理补充**：新增 `settings.autoMode.hard_deny` 可硬性阻断 auto mode classifier；`allowAllClaudeAiMcps` 可让托管环境加载 claude.ai cloud MCP connectors；`ANTHROPIC_WORKSPACE_ID` 可辅助企业环境识别；API key / `apiKeyHelper` / `ANTHROPIC_AUTH_TOKEN` 会禁用 Remote Control、`/schedule`、claude.ai MCP connectors 和 notification preferences，这一点会影响企业接入方式选择。
